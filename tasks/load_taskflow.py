...
def load(data):  # ← transform()의 return값(딕셔너리 리스트)을 인자로 받음 # XCom을 통해 자동으로 전달됨
    ...
    s3 = boto3.client(
        's3',
        aws_access_key_id=os.environ.get('AWS_ACCESS_KEY_ID'), # EC2에서 IAM role로 권한 부여 시 해당 코드 필요 없음 
        aws_secret_access_key=os.environ.get('AWS_SECRET_ACCESS_KEY'), # EC2에서 IAM role로 권한 부여 시 해당 코드 필요 없음 
